...
int main () {
  //1 //FONTI: EZT MEG KELLETT NEZNEM NETEN; JO LENNE FEJBOL TUDNI
  srand (time(NULL));
  ...
  while (ifs >> be) {
    ...
    if (be == 'F') {
      ...
      if (elozo_harom[2] == 'F') { //F-rol I-re
        if (tiszafej_szamok[0] > tiszafej_szamok[1]) { //ha a legutolso tisztafej sorozat szam nagyobb mint a legnagyobbe...
  ...
  //4
  cout.precision(2); //FONTOS: 1: cout.precision(2); | 2: << fixed << | 3: double() ...
  cout << "4. feladat" << endl << "A kiserlet soran a fej relativ gyakorisaga " << fixed <<(double) 100*kimutatas[1]/kimutatas[0] << "% volt." << endl; //FONT: fixed-et ki kell baszni oda
  ...
  //7 //FONTI: EZZEL BAXTAM EL A ELGTOBB IDOT
