...
using namespace std;
//FONTI: TODO: LEARN USING STRING::INSERT; instert(string, pos, substring, sublen); substring: string in string, sublen: len of substring
int ar(int kat) { //5
  ...
  for (int i_sor = 0; i_sor < sorok; ++i_sor) {
    ...
    for (int i_szek = 0; i_szek < szekek; ++ i_szek) {
      if ( foglaltsag[i_sor][i_szek] == 'x' ) {
         szabad_arkat_sor += 'x'; //7 //FONTOS: NEM GYAKRAN HASZNALT MODSZER: += STRINGHEZ CHAR!!
         ++eladott_jegyek;
         ++arkat_kimutatas[ (arkat[i_sor][i_szek]-'0')-1 ]; //4
         //6:
         if (i_szek > 1) { //FONTOS: FIGYELMESSEG: A NEZOTER SZELEN LEVO EGYEDULALLOKAT IS NEZNI KELL
